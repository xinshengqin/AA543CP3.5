...
class mesh(object):
    def __init__(self, xmin, xmax, n, ratio = None, xclusteredPoint = None):
        ...
        #Create nonuniform mesh
            ...
            self.n1 = int((self.xclusteredPoint-self.startPoint)/(self.endPoint-self.startPoint)*self.numberOfPoints) 
            self.n2 = self.numberOfPoints - self.n1
            self.nmax = max(self.n1,self.n2)
            self.b = (2*self.nmax-3*self.ratio-1)/(self.ratio-1)
            self.coordinates, self.gapSpaceDistribution = self.createNonuniformMesh()
        else:
            print "Wrong input."
    ...
    def createUniformMesh(self):
        ...
        list_of_point_coords.append(self.startPoint - delta_x) #add x0 at the left-most of mesh for BC
        ...
        list_of_point_coords.append(self.endPoint+delta_x)#add x_(J+1) at the right-most of mesh for BC
        ...
        list_of_point_coords = np.array(list_of_point_coords)#convert it to numpy array
        gapSpaceDistribution =  np.array(gapSpaceDistribution)
        print "size of gapSpaceDistribution:", gapSpaceDistribution.size
    ...
    def createNonuniformMesh(self):
        ...
        print "The gap space distribution is: {}.".format(gapSpaceDistribution) 
        print "Mesh generation complete."
        list_of_point_coords.insert(0,list_of_point_coords[0]*2-list_of_point_coords[1])#add x0 at the left-most of mesh for BC
        list_of_point_coords.append(list_of_point_coords[-1]*2-list_of_point_coords[-2])#add x_(J+1) at the right-most of mesh for BC
        list_of_point_coords = np.array(list_of_point_coords)#convert it to numpy array
        gapSpaceDistribution =  np.array(gapSpaceDistribution)
        return list_of_point_coords,gapSpaceDistribution
    ...
    def plot_pointVsIndex(self):
        index = np.linspace(1,self.numberOfPoints+2,self.numberOfPoints+2)
    ...
    def plot_gapSpaceVsIndex(self):
        index = np.linspace(1,self.numberOfPoints-1,self.numberOfPoints-1)
